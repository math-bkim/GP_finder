...
class Policy(torch.nn.Module):
    ...
    def step(self, state, a):
        ...
        target_n = int(state[0]*100) # distance toward 0
        a  = math.floor(target_n*(a+1)/2) + 1 # change tanh input to a positive integer from 1 to target_n
        n_state = state[:]
        n_state[0] -= Fraction(a, 100)
        n_state[1] += Fraction(1, a)
            
        if n_state[0]  < 0.02  or n_state[1] > 1:
          done = True
    ...
    def decorder(self, n, ptn, expl_noise, render_op=False):
        s = [ Fraction(n-sum(ptn),100), sum(Fraction(1,pt) for pt in ptn)]
        pi =ptn[:]
        if s[0] < 0.02 or s[1] > 1:
          done = True
        else:
          done = False
          ...
          while not done:
            ...
            pi.append(math.floor((a + 1)*temp*50)+1)
            s, _, done, _ = self.step(s, a)
        ...
        if render_op:
            print("""
                  ...
                  """ %(n, pi, sum(pi), sum(Fraction(1,pt) for pt in pi)))
        else:
            return pi
...
class GA:
    def __init__(self, hps, target_n, init_ptn = []):
        ...
        print("""
              ...
              Initial fitness :  %.3f
              """ 
              %(self.target_n, self.init_ptn, self.pop_size/1000, self.mu_rate, self.best_partition, self.best_fitness),
              file = self.f)
    ...
    def recisum(self, pi):
        return sum(Fraction(1,pt) for pt in pi)
    ...
    def run(self, upper_bound=200):
        ...
        for n_gen in range(upper_bound):
            ...
            if u_ctr >= 30:
              ...
              if self.mu_rate<.41:
                self.mu_rate +=0.05
                print("exploration_rate is now %.2f" %self.mu_rate, file=self.f)
              else:
                break
            ...
            if best_fitness > self.best_fitness:
                ...
                print(""" best candidate : %s,  
                         ...
                         , file=self.f)
                print("exploration rate is now %.2f" %self.mu_rate, file=self.f)
              
                    
            if self.best_fitness ==1000.0:
                print("successful", file=self.f)
                ...
                print("""
                      ...
                      """
                      %(n_gen+1, found, sum(found), sum(Fraction(1,pt) for pt in found))
                      , file = self.f)
...
class hp:   #collection of hyper parameters#
    def __init__(self):
        ...
        self.load_file_name = 'test'
        self.hidden = 300
        self.pop_size = int(5e3)
        self.mu_rate = 0.1
